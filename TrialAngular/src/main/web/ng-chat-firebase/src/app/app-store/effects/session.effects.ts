...
export class SessionEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(() => {
        return this.afAuth.authState
          .pipe(
            take(1),
            map((result: firebase.default.User | null) => {
              if (!result) {
      ...
      // ユーザーの認証下情報を取得
      switchMap((auth: firebase.default.User | any) => {
      ...
      switchMap((auth: firebase.default.User | any) => {
        ...
        return this.afs
          ...
          .pipe(
            ...
            catchError(this.handleLoginError(
              'loginUser', SessionActions.loginSessionsFailure
            ))
          );
      })
    )
  ...
  logoutSession$ = createEffect(() =>
    this.actions$.pipe(
      ...
      catchError(this.handleLoginError(
        'logoutUser', SessionActions.logoutSessionsFailure
      ))
    )
  );
